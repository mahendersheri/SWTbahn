...
// accelerate as soon as possible to the maximum possible speed
//and (maintining constant speed to the maximum possible distance)
// brake as quick as posssible to keep in speed limits for energy conscious behaviour.
...
func (m StandardManager) Speed(t *simulation.Train, timeElapsed time.Duration) float64 {
	...
	case simulation.ASAP:
		...
		// dtnsignal derived using Motion equation of a vehicle
	...
	// considering the power to weight ratio of acceleration at constant power in hp
	acceleration := math.Max(-t.TrainType().EmergBraking,
		math.Min(1/secs*(targetSpeed-t.Speed), math.Max(t.TrainType().StdAccel, math.Abs(t.TrainType().Enginepower)/(t.Speed*math.Abs(t.TrainType().Weight)))))
...
func targetSpeed(t *simulation.Train, secs, targetDistance, targetSpeed float64) float64 {
	...
	// standard deceleration based on the target distance and current speed
	// selecting the maximum deceleration of all possible for applying brakes as quick as possible
	// for energy conscious behaviour
	var k float64
	var t0 float64
	var stddec float64
	k = 1
	t0 = 2 / k
	stddec = math.Abs((t.Speed * t.Speed) / 2 * (targetDistance - t.Speed*t0))
	d := 0.5 * math.Pow(secs, 2) * stddec
	if targetDistance < d {
		return targetSpeed
	}
