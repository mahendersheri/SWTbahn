...
//update reverse action only to specific traintypes, Display messages in the messagelogger
//give the braking information
func (t *Train) Reverse() error {
	if t.TrainTypeCode == "cargo" {
		t.simulation.MessageLogger.addMessage(fmt.Sprintf("This train %s does not support reverse ", t.TrainTypeCode), simulationMsg)
		return errors.New(fmt.Sprintf("This train %s does not support reverse ", t.TrainTypeCode))
	}
	if t.TrainTypeCode == "UT" {
		t.simulation.MessageLogger.addMessage(fmt.Sprintf("This train %s  does not support reverse ", t.TrainTypeCode), simulationMsg)
		return errors.New(fmt.Sprintf("This train %s does not support reverse ", t.TrainTypeCode))
	}
	if t.TrainTypeCode == "cargo_db" {
		t.simulation.MessageLogger.addMessage(fmt.Sprintf("This train %s  does not support reverse ", t.TrainTypeCode), simulationMsg)
		return errors.New("This train does not support reverse ")
	}
	if t.TrainTypeCode == "cargo_bayern" {
		t.simulation.MessageLogger.addMessage(fmt.Sprintf("This train %s  does not support reverse ", t.TrainTypeCode), simulationMsg)
		return errors.New("This train does not support reverse ")
	}
	if t.TrainTypeCode == "cargo_green" {
		t.simulation.MessageLogger.addMessage(fmt.Sprintf("This train %s  does not support reverse ", t.TrainTypeCode), simulationMsg)
		return errors.New("This train does not support reverse ")
	}
	if t.TrainTypeCode == "cargo_rail" {
		t.simulation.MessageLogger.addMessage(fmt.Sprintf("This train %s  does not support reverse ", t.TrainTypeCode), simulationMsg)
		return errors.New("This train does not support reverse ")
	}
	if t.TrainTypeCode == "cargo_bahn" {
		t.simulation.MessageLogger.addMessage(fmt.Sprintf("This train %s  does not support reverse ", t.TrainTypeCode), simulationMsg)
		return errors.New("This train does not support reverse ")
	}

	if t.Speed != 0 {
		brakingforce := math.Abs(t.TrainType().Weight * t.TrainType().StdBraking)
		x := (0.5 * math.Abs(t.TrainType().Weight) * t.Speed * t.Speed) / brakingforce
		t.simulation.MessageLogger.addMessage(fmt.Sprintf("This train %s must reduce the speed from %f to zero for reverse", t.TrainTypeCode, t.Speed), simulationMsg)
		t.simulation.MessageLogger.addMessage(fmt.Sprintf("Apply brakes to stop at a stopping distance of %f meters ", x), simulationMsg)
...
func (t *Train) logTrainEntersArea() {
	...
	case -time.Minute < t.effInitialDelay && t.effInitialDelay < time.Minute:
		msg = fmt.Sprintf("Train %s with traintype %s has entered the area on time", t.ServiceCode, t.TrainTypeCode)
	case t.effInitialDelay <= -60:
		msg = fmt.Sprintf("Train %s with traintype %s has entered the area %d minutes early", t.ServiceCode, t.TrainTypeCode, t.effInitialDelay/time.Minute)
	case t.effInitialDelay >= 60:
		msg = fmt.Sprintf("Train %s with traintype %s has entered the area %d minutes late", t.ServiceCode, t.TrainTypeCode, t.effInitialDelay/time.Minute)
	}
