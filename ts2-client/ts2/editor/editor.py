...
class Editor(simulation.Simulation):
    ...
    def exportTrainTypesToFile(self, fileName):
        with open(fileName, 'w', newline='') as csvfile:
            fieldnames = [
                "code", "description", "emergBraking", "length", "maxSpeed", "stdAccel", "stdBraking", "elements", "weight","enginepower",
                "__type__"
            ]
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()
    ...
    def importTrainTypesFromFile(self, fileName):
        ...
        with open(fileName, newline='') as csvfile:
            ...
            for row in reader:
                tt = row.copy()
                numFields = ["emergBraking", "length", "maxSpeed", "stdAccel", "stdBraking", "weight","enginepower"]
                for f in numFields:
