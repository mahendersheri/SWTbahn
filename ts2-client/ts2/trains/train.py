        ...
        if self._train is not None:
            ...
            if role == Qt.DisplayRole:
                if index.column() == 0:
                    ...
                    #elif index.row() == 12 and line is not None:
                     #   return self._train.acceleration
                    else:
                        return ""
            elif role == Qt.ForegroundRole:
...
class Train(QtCore.QObject):
    ...
    def __init__(self, parameters):
        ...
        #self._acceleration = parameters.get('acceleration')
        ...
        self.ebrakeAction = QtWidgets.QAction(self.tr("emergency brake"), self)
        self.ebrakeAction.triggered.connect(self.ebrake)
        self.energyAction = QtWidgets.QAction(self.tr("energy consumed"), self)
        self.energyAction.triggered.connect(self.energy)
    ...
    def updateData(self, msg):
        ...
        #self._acceleration = msg["acceleration"]
    ...
    def showTrainActionsMenu(self, widget, pos):
        ...
        #contextMenu.addAction(self.energyAction)
        contextMenu.addAction(self.ebrakeAction)
    ...
    def ebrake(self):
        ...
        reason = None
        if self.speed == 0:
            reason = self.tr("This train is  stopped no need of emergency brake")
        if reason:
            QtWidgets.QMessageBox.warning(
                self.simulation.simulationWindow,
                self.tr("Unable to apply ebrake"),
                reason,
                QtWidgets.QMessageBox.Ok
            )
        else:

            self.simulation.simulationWindow.webSocket.sendRequest("train", "ebrake", {'id': int(self.trainId)})
    ...
    def energy(self):
      ...
      energy consumed
        ...
        reason = None
        if self.speed == 0:
            reason = self.tr("Energy consumed is ")
        if reason:
            QtWidgets.QMessageBox.warning(
                self.simulation.simulationWindow,
                self.tr("energy consumed"),
                reason,
                QtWidgets.QMessageBox.Ok
            )
